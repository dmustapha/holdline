# Holdline — C3 HERO GATE Proof (localnet mainnet-fork)
...
(`CpiDisabled`, err `0x17c0` — see DEV-015), the protective repay fires in two keeper-signed steps:
...
### Atomic enforcement (v3 — structural custody restored, INVARIANT #2)
...
## Autonomy (E-1) — fire blockTime between two logged polls
From `evidence/keeper-log.jsonl` (append-only), a fire with NO correlated human action, bracketed by
autonomous polls 15 s apart:
```
{"kind":"tick","ts":1790309346,"obligation":"2qvrAwYC…","ltvBps":8028,"marketClosed":true,"decision":"fire"}
{"kind":"fire","ts":1790309347,"sig":"3Ysw2tUB…","ltvBefore":8028,"ltvAfter":5955}
{"kind":"tick","ts":1790309362,"obligation":"2qvrAwYC…","ltvBps":5955,"marketClosed":true,"decision":"hold"}
{"kind":"tick","ts":1790309377,"obligation":"2qvrAwYC…","ltvBps":5955,"marketClosed":true,"decision":"hold"}
{"kind":"tick","ts":1790309392,"obligation":"2qvrAwYC…","ltvBps":5955,"marketClosed":true,"decision":"hold"}
```
The fire (ts 1790309347) falls **between** poll 1790309346 and poll 1790309362. After the fire the
keeper self-corrects: LTV is back under trigger, so subsequent ticks decide **hold** (no re-fire).
Market-hours gate: evaluated at a real closed-hours moment (weekend `2026-09-27T12:00:00Z` via the
disclosed `HOLDLINE_NOW` demo clock, since the host system clock can't be moved) — `isClosed()`'s
weekend rule is unchanged.
